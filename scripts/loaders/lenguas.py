...
quienes las hablan. Sale de las siete muestras con clave de lengua (1990,
2000, 2005, 2010, 2015, 2020 y 2025), a nivel alcaldía: ningún producto
censal publica la lengua por AGEB ni por manzana.

Salidas:
  src/data/lenguas_alcaldia.csv
    anio, nivel, cve, nombre, lengua (clave INALI), lengua_nombre, familia,
    sexo, num, den, casos, ee, fuente, universo
    num = hablantes de esa lengua; den = población del universo de habla
...
Las claves históricas de 1990-2005 se traducen al INALI con
catalogo_lenguas_historico.csv (medida en 2010, donde vienen las dos).

Cotejos: 2015 reproduce la tabla 1 del documento de la Secretaría de Cultura
(náhuatl 38 549, mixteco 15 920, otomí 13 764, ...) y 43 claves; 2020, el
orden de la tabla 3 de la SEPI.
"""
import os
import sys
...
from muestras import CARGADORES, EDAD_HLI, SIN_DISENO, comprobar  # noqa: E402
...
ANIOS = (1990, 2000, 2005, 2010, 2015, 2020, 2025)
...
def main():
    ...
    for anio in ANIOS:
        ...
        sel = ",\n  ".join(f"(lengua = '{k}') AS y_l{k}, (hli IS NOT NULL AND edad >= {edad}) AS u_l{k}" for k in lenguas)
        ...
        ind = [{"clave": f"l{k}", "tema": "lenguas", "indicador": k, "universo": f"Población de {edad} años y más"} for k in lenguas]
        ...
        for _, r in largo.iterrows():
            ...
            alc.append({"anio": anio, "nivel": nivel, "cve": r["cve_mun"],
                        ...
                        "ee": None if anio in SIN_DISENO else r["ee"], "fuente": FUENTE_MUESTRA[anio],
                        "universo": f"Población de {edad} años y más"})
        ...
        if "0211" in tot.index:
            anotar_calculado("lenguas", f"nahuatl_{anio}", tot["0211"], f"hablantes de náhuatl en la CDMX, muestra {anio}")
